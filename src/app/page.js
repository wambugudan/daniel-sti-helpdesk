...
export default function Home() {
  ...
  // The line below was for dev purposes and is now commented out as intended.
  // const [user, setUser] = useState(true)

  // Determine the link path and text based on the user's session and role
  let dashboardLink = "/submissions";
  let buttonText = "Go to Dashboard";

  if (session?.user?.role === 'ADMIN') {
    dashboardLink = "/admin"; // Admin users go to /admin
    buttonText = "Go to Admin Dashboard"; // Text for admin users
  } else if (session?.user) {
    dashboardLink = "/submissions"; // Regular users go to /submissions
    buttonText = "Go to Dashboard"; // Text for regular users
  } else {
    dashboardLink = "/login"; // No user logged in, go to login
            ...
            <div className="mx-auto text-center lg:max-w-4xl">
              ...
              {showMore ? (
                <div>
                  ...
                  <p className="mt-8 px-10 text-gray-700">
                    This helpdesk has been made possible through the Science
                    Granting Councils Initiative (www.sgcia.org) and
                    funding provided to the African Centre for Technology
                    Studies (ACTS) to support African Science Councils with
                    support in utilising data and evidence for policy and
                    decision making.
              ...
              <div className="mt-16 flex flex-wrap justify-center gap-y-4 gap-x-6">
                {/* Now using the dynamically determined dashboardLink and buttonText */}
                <Link
                  href={dashboardLink}
                  className="relative flex h-11 w-full items-center justify-center px-9 before:absolute before:inset-0 before:rounded-md before:bg-primary before:transition before:duration-300 hover:before:scale-105 active:duration-75 active:before:scale-95 sm:w-max"
                >
