// File: ContractCard.jsx
'use client'; // Essential for client-side components using hooks like useTheme
        ...
        <CardWrapper>
            ...
            <div className="space-y-2 text-sm">
                <p className="flex items-center gap-2">
                    ...
                    {/* Ensure contract.userId exists before linking */}
                    {contract.userId ? (
                        <Link
                            href={`/profile/${contract.userId}`}
                            className={clsx(
                                "font-medium hover:underline",
                                theme === "dark" ? "text-blue-400 hover:text-blue-300" : "text-blue-600 hover:text-blue-700"
                            )}
                        >
                            {contract.user?.name || contract.user?.email || 'Unknown Client'}
                        </Link>
                    ) : (
                        // Fallback if userId is not available for some reason
                        <span className={clsx(
